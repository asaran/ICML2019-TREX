...
bash_file_name = 'PPO_'+user_type+'_conv'+conv_layer+'_'+gaze_loss_type+'.sh'
...
for env in env_names:
  for seed in seeds:
    ...
    f.write("screen -S "+screen_name+" -X stuff \"CUDA_VISIBLE_DEVICES="+gpu_id+" OPENAI_LOG_FORMAT='stdout,log,csv,tensorboard' OPENAI_LOGDIR=path_to_logs/"+user_type+"s/"+env[0]+"_"+user_type+"_conv"+conv_layer+"_"+gaze_loss_type+"_"+seed+" python -m baselines.run --alg=ppo2 --env="+env[1]+"NoFrameskip-v4 --custom_reward pytorch --custom_reward_path learned_models/"+user_type+"s/"+env[0]+"_"+user_type+"_"+gaze_loss_type+"_conv"+conv_layer+" --seed "+seed+" --num_timesteps=5e7 --save_interval=500 --num_env 9\n")
    f.write("\"\n")
    i+=1

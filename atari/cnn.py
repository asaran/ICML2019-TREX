...
class Net(nn.Module):
    ...
    def cum_return(self, traj, gaze_conv_layer=0):
        ...
        if self.gaze_loss_type is not None and gaze_conv_layer!=0:
            ...
            min_x = min_x.reshape(traj_length,1).repeat(1,gaze_size).unsqueeze(-1).expand(traj_length,gaze_size,gaze_size)
            max_x = max_x.reshape(traj_length,1).repeat(1,gaze_size).unsqueeze(-1).expand(traj_length,gaze_size,gaze_size)
    ...
    def forward(self, traj_i, traj_j, gaze_conv_layer=0):
        ...
        # print('traj_i inside forward: ',traj_i.shape)

...
for game in env_names:
    for seed in seeds:
        for weight in weights:
            d = 'reward_models_AAAI2020/'+game+'_seed'+'_'+seed+'two-data'
            
            #if not os.path.isdir('../'+d):
            #    os.mkdir('../'+d)

            #f.write('#SBATCH --job-name asterix_'+weight+'_'+seed)
            screen_name = 'two-data'+game+'_seed'+seed
            bash_file_name = 'gaze_pred/reward_'+game+'_human_seed'+seed+'_two-data.sh'

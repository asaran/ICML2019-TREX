...
for game in env_names:
    for seed in seeds:
        for weight in weights:
            d = 'reward_models_AAAI2020/gazePred_'+game+'_KL_'+weight+"_seed"+seed
            #if not os.path.isdir('../'+d):
            #    os.mkdir('../'+d)

            #f.write('#SBATCH --job-name asterix_'+weight+'_'+seed)
            screen_name = game+'_seed'+seed+'_'+weight
            bash_file_name = 'gaze_pred/reward_'+game+'_human_kl_seed'+seed+'_'+weight+'.sh'
